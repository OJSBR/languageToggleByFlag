...
This is an OJSBR adaptation of the original `languageToggleByFlag` plugin by
**Lepidus Tecnologia** (<https://github.com/lepidus/languageToggleByFlag>). If your change
is not specific to PKP 3.5, please consider contributing it upstream as well.

## Reporting issues

- Open an issue describing the problem or suggestion.
- Include your **OJS/OMP/OPS version**, the **plugin version** (see `version.xml`), and steps
...
| Branch | Target |
|--------|--------|
| `stable-3_5_0` | OJS/OMP/OPS 3.5.x |
...
2. Keep the repository layout intact: the repo root **is** the plugin folder (so
   `version.xml` stays at the root), and it installs into `plugins/blocks/languageToggleByFlag/`.
3. Follow the existing code style and the
   [PKP coding conventions](https://docs.pkp.sfu.ca/dev/documentation/en/coding) —
   namespaced classes (`APP\plugins\...`, PSR-4), etc.
...
Esta é uma adaptação da OJSBR do plugin original `languageToggleByFlag` da
**Lepidus Tecnologia** (<https://github.com/lepidus/languageToggleByFlag>). Se a sua mudança
não for específica do PKP 3.5, considere contribuí-la também no repositório original.

### Relatando problemas

- Abra uma *issue* descrevendo o problema ou a sugestão.
- Informe a **versão do OJS/OMP/OPS**, a **versão do plugin** (veja `version.xml`) e o passo a
...
`stable-3_5_0` (OJS/OMP/OPS 3.5.x). **Baseie seu trabalho — e abra o pull request — na branch
que corresponde à versão do PKP que você está mirando.**
...
2. Mantenha o layout do repositório: a raiz do repo **é** a pasta do plugin (o `version.xml`
   fica na raiz), e ele instala em `plugins/blocks/languageToggleByFlag/`.
3. Siga o estilo do código e as
   [convenções de código da PKP](https://docs.pkp.sfu.ca/dev/documentation/en/coding) —
   classes com namespace (`APP\plugins\...`, PSR-4) etc.
